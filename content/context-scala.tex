...
Recall from \fref{sec:cop}, that layers encapsulate \emph{partial} behaviors which are in scope only when they are determined to be by the language's activation mechanism.
In our implementation, we define layers using the \scode{PredicatedFunction} abstraction. A \scode{PredicatedFunction} is the combination of an activation condition and a \scode{Behavior}.
The activation condition is a predicate that depends on the state of the current execution context, while the \scode{Behavior} is a function that may manipulate said state and return some result.
A \scode{PredicatedFunction} is then a partial function that depends on a given state value (\ie context), whose \scode{Behavior} can only be applied if the activation predicate evaluates to \scode{true} for said state value.

A \scode{Behavior} is represented through the \scode{State} Monad: a function that takes some state and returns a product containing the new state and a result, as shown in \fref{lst:state-type}
...
Additionally, the \scode{State} type has the \scode{Monad} instance shown in \fref{lst:state-monad}, specifying how to chain two state functions sequentially,
...
We saw in \fref{sec:cop} that multiple layers can be composed together to form new behavioral variations,
and that each layer within the composition can access the behavior from a previously active layer.
To achieve the same functionality with \scode{PredicatedFunctions}, we define a \scode{Behavior} to be a \scode{State} function that depends on a previously active computation,
...
which is easier to implement. Rather than having a special \scode{proceed} construct that would require meta-programming to implement,
we use Higher-Order Functions to express state manipulations that may depend on previous computations.

Finally, with these definitions in place, A \scode{PredicatedFunction} is defined in \fref{lst:predicated-function} as a partial \scode{Behavior}
...
We use the \scode{Monad} instance of the \scode{State} type to compose various \scode{Behaviors}.
There are two strategies to combine two \scode{PredicatedFunctions}.
...
at the same time, both Behaviors are chained using \scode{flatMap}. If neither is active, then none of the Behaviors are executed, and the result of the
...
shown in \fref{lst:predicated-function-creation-example}. This example shows three \scode{PredicatedFunction} that
depend on a context state of type \scode{Int}, and return a result of type \scode{String}.
Each function defines a \scode{Behavior} that may manipulate the state in some way and return a result.
For instance, \fref{ln:predicated-function-creation-example.fun2} shows that, when \scode{Fun2} is active, it adds 10 to the \scode{state} value, and it appends the \scode{"Hello from Fun2."} string to
the result of the most recently active \scode{PredicatedFunction}.
...
using the two composition strategies: \scode{sequence} (\fref{ln:predicated-function-composition-example.fun4}) and \scode{oneOf} (\fref{ln:predicated-function-composition-example.fun5})
...
Given an initial state of 1, the result of the \scode{sequence} composition in \fref{ln:predicated-function-composition-example.run1} is \scode{(9, "Hello from Fun2. Hello from Fun3. ")}
because only \scode{Fun2} and \scode{Fun3} are active for that particular value. \scode{Fun2} first adds 10 to the initial state, which then activates the Behavior of \scode{Fun3} because its predicate requires that \scode{state > 10}.
The result of the \scode{oneOf} composition (\fref{ln:predicated-function-composition-example.run2}), on the other hand, is \scode{(11, "Hello from Fun2. ")} because only the first active function in the composition is executed.
...
The \scode{PredicatedFunction} abstraction is enough to construct \ac{COP} programs, however it still has many of the
...
\fref{lst:predicated-function-composition-example}. The value of \scode{run1} would be different if the programmer composed the functions in a different order;
for example, the composition \scode{sequence(Fun1, Fun3, Fun2)} yields \scode{(11, "Hello from Fun2. ")}, when applied to the pair of initial state and result \scode{(1, "")}.
...
The main modification that is done to the \scode{PredicatedFunction}, is to simply add the activation predicate as a precondition to the \scode{run} function, as show in \fref{lst:predicated-function-run-precondition}.
...
Adding the pre-condition to the \scode{run} function's \scode{state} argument means that it can be only applied if the caller provides an \emph{evidence} that the \scode{isActive} predicate holds for the given \scode{state} input.
Such specification also allows the programmer to bring the assumptions from the \scode{isActive} predicate into scope, so that they can be used when defining the program's logic. \fref{lst:predicated-function-run-precondition-example} shows
a simple example of how an optional value can be safely accessed from within the \scode{run} function's scope, due to the \scode{isActive} precondition.
The \scode{state} function parameter is refined to \lstinline|Option[BigInt]{v : v.isInstanceOf[Some] && v.get > 10}|, guaranteeing that \scode{state.get} will never throw an exception as it should always be an instance of the \scode{Some} type.
...
An instance of the \scode{Chainable} type serves as an evidence that, for the given \scode{first} and \scode{second} predicated functions,
the \scode{isChainable} predicate holds for all possible values of \scode{previous: A} and \scode{state: S}. The property states that the
activation condition of the \scode{first} function implies that, when applying it to the previous result and state, the \scode{second} function will be active for the new returned state.
The type of the \scode{isChainable} property is shown in Equation \ref{eqn:isChainable}.
...
Note that the \scode{first.run} call on \fref{ln:predicated-function-chainable.first.run} must necessarily happen inside the right hand side block of the implication (\scode{==>}) because the \scode{run} function's precondition states that the \scode{isActive} predicate
...
When instantiating a new \scode{Chainable} instance with two concrete Predicated Functions,
Stainless will automatically verify that the \scode{isChainable} law holds for those particular functions.
If, for instance, \scode{RefinedFun1} subtracted 10 instead of 9 to the input state (\scode{state.get - 10}), then
the programmer would get a compilation error because there exists a \scode{state} value for which the \scode{isChainable} property does not hold;
the SMT solver will also return an example of a value that makes the predicate false (in this case \scode{state =  11})
