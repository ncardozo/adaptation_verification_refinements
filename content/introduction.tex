...
on the information gathered from the execution environment. Layer activation and deactivation 
respectively make available and unavailable to the executing program, the behavior and state defined 
within the layer. The layer activation mechanism allows programmers to express variability, without 
...
the program~\cite{martou24}, ultimately leading to bugs or execution anomalies.
Validating the correctness of a \ac{COP} program has been a topic of interest in recent years, 
and several solutions have been proposed, as for example the use of debuggers~\cite{uchio11}, formal 
...
techniques~\cite{martou24}. However, currently, no application of software verification 
...
possible partial configurations. 
...
interactions between different behavioral components. The editor is developed in two iterations, one with 
out Refinement Types, and one with Refinement Types, as an ablation of the usefulness of Refinement 
Types. We show how the use of Refinement Types assures a consistent and complete \ac{COP} 
program, for our case study.
...
we extend Scala with \ac{COP} capabilities primarily because:
