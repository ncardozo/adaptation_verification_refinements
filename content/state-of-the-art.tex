...
way of specifying its pre- and post-conditions of a function. In fact, 
...
There have been several usages of Refinement Types for developing reliable distributed systems with 
...
\ac{COP} program, specially when using the implicit activation and scoping mechanism is used. 
Program flow can become complicated very quickly when layers depend on partial methods provided 
by other layers, as the result of a call does not only depend on the activation conditions of the partial 
method called, but also on the activations of all other layers. The problem is finding  a way of 
...
As previously mentioned, a recent validation approach puts forward the use of combinatorial testing to 
...
that cannot be detected if the heuristic does not generate one particular transition triggering the error.

Using Refinement Types, it is possible to statically verify properties about layer configurations to 
guarantee correct behavior without the need to explore the full space of combinations.
Refinement Types provide a specification for transitions between valid program configurations; that is, 
can verify there are no incomplete program traces due to missing partial method dependencies. Using 
