...
One of the goals of Refinement Types is to seamlessly incorporate formal methods 
into mainstream software, thanks to its automatic verification capabilities, the flexibility it provides 
...
\begin{scala}[numbers=none,label=lst:refinement-nat-example, caption=Natural numbers definition by refining integers with Refinement Types]
  type Nat = Int{v | 0 <= v}
\end{scala}

The types of the parameters and return values of a function can also be refined, effectively providing a 
way of specifying its pre- and post-conditions; in fact, 
...
To illustrate how Type Checking works with Refinement Types, we present an example of the simply typed lambda calculus with Refinement Types~\cite{jhala21}.
\begin{align*}
...
would arise from asserting that $Type : Type$, we can instead state $Type : Kind$~\cite{ragde20}.
...
All types and their refinements must be first organized in terms of the aforementioned Kind system. 
To achieve this, the type system has well-formedness rules that state how Kinds are 
...
extended with the refinement of type $b$ (that is, $ x : b \{x \; | \; p \}$), if the constraint 
...
term of type $int\{v  \mid 10 \leq v\}$ whenever we require a term of type $int\{v \mid 0 \leq v\}$.
...
Knowing that $inc(x)$ has type $int\{v = x + 1\}$, we need to conclude that it also has type 
$int\{0 \leq v\}$. We use the Subtyping rules to prove that $inc(x)$ has type $int\{0 \leq v\}$ by 
generating the \ac{VC} constraint: 
$\forall x : int \: . \: 0 \leq x \Rightarrow \forall v : int \: . \: v = x + 1 \Rightarrow 0 \leq v$; which is 
deemed true by the \ac{SMT} solver. Note how the Ent-ext rule is used to construct the final \ac{VC} 
constraint, and Ent-emp is used to call the \ac{SMT} solver.
...
the specified type, often using a DSL defined by the Refinement Type language.

There have been several usages of Refinement Types for developing reliable systems. The formalization of distributed systems with 
LiquidHaskell is a good example of the  applicability of Refinement Types in software 
development~\cite{kuper22,liittschwager25}. Refinement Types are used to formally verify that CRDT 
implementations are correct, by proving that the application of operations to update the CRDT state is 
commutative. A CRDT is defined as a Typeclass containing the type definitions of the operation and 
the proof obligation of its commutativity~\cite{kuper22}. Implementors of a CRDT instance are 
compelled to write the commutativity proof, otherwise the program will not compile. Having the 
verification system embedded within the same language that the programs are written on, is one of the 
main advantages highlighted by the proponents of Refinement Type Systems~\cite{jhala21}. Unlike 
many software verification tools that formally reason about a model that is separate from executable 
code, Refinement Types allow developers to verify the actual code that gets executed directly.
...
Stainless~\cite{hamza19} is a refinement type-based verifier written in Scala used to reason about 
Scala programs. We use Stainless in this work for the verification of \ac{COP} programs. In particular, 
Stainless is based on System FR, which is a calculus that supports System F polymorphism, 
Refinement Types, and recursive types~\cite{hamza19}. Stainless is able to verify a purely functional 
subset of Scala that is SMT decidable. Moreover, Stainless is also capable of verifying an imperative 
fragment of the language which uses mutable objects and loops~\cite{hamza19}. The following 
example is a verified specification of the factorial function with Stainless, taken from the framework's 
documentation.
...
refinements similar to \scode{BigInt\{v : v >= 0\}} in both cases. These refinements are type 
...
%%
\subsection{Verifying Context-Oriented Programs with Refinement Types}
\label{sec:problem-of-verifying-cop}

Now that we have presented Refinement Types, we make an argument for the difficulties in verifying 
\ac{COP} programs and posit Refinement Types as an option to overcome to said difficulties.

In general, the result of a partial method depends on the order of the composition of layers, and the 
current active layers. This property makes it difficult to reason about the expected behavior of a 
\ac{COP} program, specially when using the implicit activation and scoping mechanism. 
Program flow can become complicated very quickly when layers depend on partial methods provided 
by other layers, as the result of a call does not only depend on the activation conditions of the  
method being called, but also on the activations of all the other layers. The problem is finding  a way of 
validating that the observed behavior is as expected for all possible interactions and activation 
configurations. In practice, the space of possible layer combinations is exponential, making such 
approach unfeasible.

A recent validation approach puts forward the use of combinatorial testing to 
generate a set of transitions between partial configurations, in order to detect incorrect behavior that 
arise from the activation of specific layers~\cite{martou24}. Combinatorial testing generates transitions 
between configurations in accordance to a given heuristic. Heuristics are used to maximize the 
coverage of interactions while optimizing for performance, as it is not practical to explore the complete 
space of all possible transitions. The drawback of this is, however, that there may be some error cases 
that cannot be detected if the heuristic does not generate a particular transition that could trigger an error.

Using Refinement Types, it is possible to statically verify properties about layer configurations to 
guarantee correct behavior without needing to explore the full space of combinations.
Refinement Types provide a specification for transitions between valid program configurations; that is, 
they can verify that there are no incomplete program traces due to missing partial method dependencies. Using 
Refinement Types we can enforce program completeness by design, as incomplete programs lead to 
compile-time errors.
